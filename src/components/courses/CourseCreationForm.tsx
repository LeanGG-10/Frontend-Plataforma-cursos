...
import { UploadCloud, Image as ImageIcon, X, Loader2, AlertCircle, Save, Send, BookOpen, CheckCircle2, Trash2, Plus, Lock } from 'lucide-react';
...
export default function CourseCreationForm({ courseId, categories }: Props) {
 ...
 if (courseId) {
 await coursesService.updateCourse(courseId, basePayload);
 setSubmitStatus({
 visible: true,
 status: 'SUCCESS',
 ...
 redirectUrl: userRole === 'PROFESOR' ? '/mis-cursos' : '/cursos'
 });
 }
 } catch (err: any) {
 setSubmitStatus(prev => ({ ...prev, visible: false }));
 ...
 if (isLoadingCourse) {
 return (
 <div className="bg-slate-50 rounded-2xl p-12 flex flex-col items-center justify-center min-h-[400px]">
 <Loader2 className="w-8 h-8 text-indigo-500 animate-spin mb-4" />
 <p className="text-slate-500 ">Cargando detalles del curso...</p>
 </div>
 );
 }

 return (
 <div className="bg-slate-50 rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8 font-sans">
 ...
 {rejectionReason && <p className="text-xs mt-1 mb-2">Motivo: {rejectionReason}</p>}
 <p className="text-xs mt-2 font-medium">
 Nota: Recuerda que para volver a enviar el curso a revisión debes guardarlo como borrador primero otra vez.
 </p>
 </div>
 </div>
 )}
 ...
 <div className="mb-6 p-4 bg-amber-50 text-amber-700 rounded-xl flex items-start justify-between gap-3 border border-amber-200 ">
 ...
 className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold rounded-lg transition-colors shrink-0 cursor-pointer"
 >
 Cancelar Revisión
 </button>
 )}
 ...
 className={`px-4 py-3 font-medium text-sm transition-colors border-b-2 whitespace-nowrap cursor-pointer ${activeTab === 'INFO' ? 'border-indigo-600 text-indigo-600 ' : 'border-transparent text-slate-500 hover:text-slate-700 :text-slate-300'}`}
 >
 Información General
 </button>
 <button 
 onClick={() => setActiveTab('STRUCTURE')}
 className={`px-4 py-3 font-medium text-sm transition-colors border-b-2 whitespace-nowrap cursor-pointer ${activeTab === 'STRUCTURE' ? 'border-indigo-600 text-indigo-600 ' : 'border-transparent text-slate-500 hover:text-slate-700 :text-slate-300'}`}
 >
 Temario y Módulos
 </button>
 </div>
 ...
 className={`w-full px-4 py-2.5 rounded-xl border bg-slate-50 text-slate-900 transition-all focus:outline-none focus:ring-2 ${
 errors.title 
 ...
 onChange={handleInputChange}
 className={`w-full px-4 py-3 rounded-xl border bg-slate-50 text-slate-900 transition-all focus:outline-none focus:ring-2 resize-none ${
 errors.description 
 ? 'border-red-500 focus:ring-red-500/20' 
 : 'border-slate-200 focus:border-indigo-500 focus:ring-indigo-500/20'
 }`}
 ...
 onChange={handleInputChange}
 className="w-full px-4 py-2.5 rounded-xl border bg-slate-50 text-slate-900 transition-all focus:outline-none focus:ring-2 focus:border-indigo-500 focus:ring-indigo-500/20 appearance-none cursor-pointer"
 >
 ...
 className="flex-1 px-4 py-2 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 transition-all focus:outline-none focus:ring-2 focus:border-indigo-500 focus:ring-indigo-500/20"
 />
 <button
 type="button"
 onClick={() => confirmRemoveListItem('learning_objectives', i)}
 className="p-2.5 text-slate-400 hover:text-red-500 hover:bg-red-50 :bg-red-500/10 rounded-xl transition-colors cursor-pointer"
 ...
 className="text-sm text-indigo-600 font-medium flex items-center gap-1 hover:underline cursor-pointer"
 >
 ...
 <div className="space-y-1.5">
 <label htmlFor="instructor" className="block text-sm font-medium text-slate-700 flex items-center justify-between">
 <span>Instructor <span className="text-red-500">*</span></span>
 {userRole === 'PROFESOR' && <span className="text-xs text-[#B8935A] font-medium">· asignado automáticamente</span>}
 </label>
 ...
 userRole === 'PROFESOR'
 ? 'border-slate-300 border-dashed bg-transparent text-slate-500 cursor-not-allowed pr-10'
 ...
 <div className="space-y-1.5">
 <label htmlFor="courseCategoryId" className="block text-sm font-medium text-slate-700 ">
 ...
 onChange={handleInputChange}
 className={`w-full px-4 py-2.5 rounded-xl border bg-slate-50 text-slate-900 transition-all focus:outline-none focus:ring-2 appearance-none cursor-pointer ${
 errors.courseCategoryId ? 'border-red-500' : 'border-slate-200 focus:border-indigo-500 focus:ring-indigo-500/20'
 }`}
 >
 ...
 onDrop={handleDrop}
 className={`relative flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-2xl transition-all overflow-hidden bg-slate-50 
 ${previewUrl ? 'border-indigo-500/50' : 'border-slate-300 hover:border-indigo-400 hover:bg-slate-100 :bg-slate-800/50'}
 ...
 <span className="font-semibold text-indigo-600 ">Sube un archivo</span> o arrástralo
 </p>
 ...
 <div className="space-y-1.5">
 <label htmlFor="price" className="block text-sm font-medium text-slate-700 ">Precio (USD)</label>
 <input
 type="number"
 id="price"
 name="price"
 ...
 onChange={handleInputChange}
 className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none"
 />
 </div>
 <div className="space-y-1.5">
 ...
 onChange={handleInputChange}
 className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-900 transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none appearance-none cursor-pointer"
 >
 ...
 <div className="space-y-1.5">
 <label htmlFor="duration" className="block text-sm font-medium text-slate-700 ">Horas Totales</label>
 <input
 type="number"
 id="duration"
 name="duration"
 ...
 {formData.status !== 'PENDING_REVIEW' && (
 <div className="mt-8 pt-6 border-t border-slate-200 flex flex-col-reverse sm:flex-row justify-end gap-4 items-center">
 {/* CA-21: Loading state buttons */}
 <button
 type="button"
 disabled={isSubmitting || uploadingImage}
 onClick={(e) => onActionClick(e, 'DRAFT')}
 className="w-full sm:w-auto px-6 py-2.5 rounded-xl font-medium text-white bg-[#B8935A] hover:bg-[#A3804C] transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
 >
 ...
 className="w-full sm:w-auto px-6 py-2.5 rounded-xl font-medium text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm shadow-indigo-200 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
 >
 ...
 className="w-full sm:w-auto px-6 py-2.5 rounded-xl font-medium text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm shadow-indigo-200 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
 >
 ...
 <h3 className="text-lg font-medium text-slate-900 mb-2">Guarda tu curso primero</h3>
 <p className="text-slate-500 max-w-md">
 Guarda la información general del curso como borrador para comenzar a agregar módulos y lecciones.
 </p>
 <button 
 onClick={() => setActiveTab('INFO')}
 className="mt-6 px-6 py-2.5 bg-transparent border border-[#B8935A] text-[#B8935A] hover:bg-[#B8935A] hover:text-white rounded-xl font-medium transition-colors"
 >
 ...
 <div className="bg-slate-50 rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-slate-200 ">
 <div className="p-6">
 <div className="w-12 h-12 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center mb-4">
 <AlertCircle size={24} />
 </div>
 <h3 className="text-xl font-semibold text-slate-900 mb-2">Posible duplicado</h3>
 <p className="text-slate-600 text-sm">{duplicateMessage}</p>
 </div>
 <div className="bg-slate-50 px-6 py-4 flex justify-end gap-3 border-t border-slate-200 ">
 <button
 type="button"
 onClick={() => setShowDuplicateModal(false)}
 className="px-4 py-2 rounded-lg font-medium text-slate-700 bg-slate-50 border border-slate-300 hover:bg-slate-50 :bg-slate-700 transition-colors"
 >
 Cancelar
 </button>
 <button
 ...
 <h3 className="text-xl font-semibold text-slate-900 mb-2">Confirmar cambios</h3>
 ...
 className="px-4 py-2 rounded-lg font-medium text-slate-700 bg-slate-50 border border-slate-300 hover:bg-slate-50 :bg-slate-700 transition-colors cursor-pointer"
 >
 Cancelar
 </button>
 <button
 ...
 submitStatus.status === 'LOADING' ? 'bg-indigo-100 text-indigo-600' : 'bg-green-100 text-green-600'
 }`}>
 ...
 <p className="text-slate-600 text-sm">{submitStatus.message}</p>
 </div>
 ...
 className="w-full px-4 py-2.5 rounded-xl font-medium text-white bg-indigo-600 hover:bg-indigo-700 transition-colors cursor-pointer"
 >
 Continuar
 </button>
 </div>
 ...
 className="px-4 py-2 rounded-lg font-medium text-white bg-red-600 hover:bg-red-700 transition-colors cursor-pointer"
 >
 Eliminar
 </button>
 </div>
 ...
 <h3 className="text-xl font-semibold text-slate-900 mb-2">¡Completado!</h3>
 <p className="text-slate-600 text-sm">{successModal.message}</p>
 </div>
 <div className="bg-slate-50 px-6 py-4 flex justify-end gap-3 border-t border-slate-200 ">
 <button
 type="button"
 ...
 <h3 className="text-xl font-semibold text-slate-900 mb-2">Enviar a revisión</h3>
 <p className="text-slate-600 text-sm">
